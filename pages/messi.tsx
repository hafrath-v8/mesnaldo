...
const TROPHIES = [
  ...
  { name: "Laureus Sportsman of the Year", count: 2, icon: "🏆" },
  { name: "Pichichi Trophy", count: 8, icon: "👟" },
  { name: "FIFA FIFPro World XI", count: 17, icon: "🌟" },
]

export default function MessiProfile({ careerStats, allMatches, clubBreakdown, goalsByYear, goalsByCompetition, totalTrophies }: MessiPageProps) {
  ...
  const opponents = useMemo(() => {
    const opps = [...new Set(allMatches.map(m => m.opponent).filter(Boolean))]
    return ["all", ...opps.sort()]
  }, [allMatches])
  ...
  const filteredMatches = useMemo(() => {
    ...
    if (matchFilter !== "all") {
      matches = matches.filter(m => m.competition === matchFilter)
    }
      ...
      <div className="bg-black">
          ...
          <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-20 relative">
            <div className="flex flex-col sm:flex-row items-center gap-6 sm:gap-10 text-center sm:text-left">
              ...
              <div>
                ...
                <h1 className="text-3xl sm:text-4xl lg:text-5xl font-black text-white tracking-tight">Lionel Messi</h1>
                <p className="text-gray-400 mt-2 text-lg">Forward · Inter Miami</p>
                <div className="flex flex-wrap items-center gap-3 mt-3 text-xs text-gray-500">
                  <span>Born: June 24, 1987 (37 yrs)</span>
                  <span>·</span>
                  <span>1.70m · Left Foot</span>
                  <span>·</span>
                  <span>#10</span>
                </div>
              </div>
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12 space-y-8">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  {clubBreakdown.map((club, i) => (
                    <motion.div key={i} initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.05 }}
                      ...
                      <p className="text-2xl font-black text-blue-400">{club.goals.toLocaleString()}</p>
                ...
                <div className="space-y-2">
                  {goalsByCompetition.slice(0, 8).map((comp, i) => (
                    <div key={i} className="flex items-center gap-3 py-2.5 px-4 rounded-xl hover:bg-gray-900/30">
                      ...
                      <span className="text-sm font-bold text-blue-400">{comp.goals}</span>
                      <div className="w-16 h-1.5 bg-gray-800 rounded-full overflow-hidden">
                        <motion.div initial={{ width: 0 }} whileInView={{ width: `${(comp.goals / totalGoals) * 100}%` }} viewport={{ once: true }} className="h-full bg-blue-500 rounded-full" />
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <div className="space-y-4">
                        ...
                        <tr key={m.id} className="border-b border-gray-700/20 hover:bg-gray-800/20 transition-colors">
                          ...
                          <td className="py-2.5 px-3 text-center">
                            <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${
                              m.result === "W" ? "bg-emerald-500/10 text-emerald-400" : 
                              m.result === "D" ? "bg-amber-500/10 text-amber-400" : 
                          ...
                          <td className="py-2.5 px-3 text-center text-xs">
                            {m.goals > 0 ? <span className="text-emerald-400 font-bold">{m.goals}</span> : <span className="text-gray-600">-</span>}
                          </td>
                          <td className="py-2.5 px-3 text-center text-xs">
                            {m.assists > 0 ? <span className="text-blue-400 font-bold">{m.assists}</span> : <span className="text-gray-600">-</span>}
                          </td>
                        </tr>
                      ))}
                    </tbody>
              ...
              {totalPages > 1 && (
                <div className="flex items-center justify-center gap-2">
                  ...
                  <button onClick={() => setMatchPage(p => Math.min(totalPages, p + 1))} disabled={matchPage === totalPages}
                    className="px-3 py-1.5 text-xs rounded-lg bg-gray-900 border border-gray-800 text-gray-400 disabled:opacity-30 hover:text-white">
                    Next →
                  </button>
                </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
              {TROPHIES.map((t, i) => (
                <motion.div key={i} initial={{ opacity: 0, scale: 0.9 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ delay: i * 0.03 }}
                  ...
                  <p className="text-lg font-black text-blue-400">{t.count}x</p>
                </motion.div>
              ))}
            </div>
          )}
                ...
                <div className="h-64 sm:h-72">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={goalsByYear}>
                      ...
                      <Bar dataKey="goals" fill="#3B82F6" radius={[6, 6, 0, 0]} />
                    </BarChart>
                  </ResponsiveContainer>
                </div>
              </div>
          ...
          <div className="flex items-center justify-center gap-4 pt-8 border-t border-gray-800">
            <Link href="/ronaldo" className="px-6 py-3 bg-red-500/10 border border-red-500/20 rounded-xl text-sm text-red-400 hover:bg-red-500/20 transition-colors">
...
export const getServerSideProps: GetServerSideProps = async () => {
  try {
    const { data: careerStats } = await supabase.from("career_stats").select("*").eq("player_id", 1).single()
    const allMatches = await fetchAllMatches(1)
    ...
    return {
      props: { careerStats: careerStats || {}, allMatches, clubBreakdown, goalsByYear, goalsByCompetition, totalTrophies: 48 },
    }
  } catch (e) {
