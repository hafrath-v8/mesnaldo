    ...
    <>
      ...
      <div className="bg-black min-h-screen flex">
        ...
        <button 
          ...
          className="lg:hidden fixed top-4 left-4 z-50 p-2 bg-gray-900 border border-gray-700 rounded-lg text-gray-400"
        >
        ...
        {/* Sidebar */}
        <div className={`w-56 bg-gray-900 border-r border-gray-800 p-4 flex flex-col flex-shrink-0 fixed lg:relative z-50 h-screen transition-transform duration-300 ${
          ...
          <nav className="space-y-1 flex-1">
            {tabs.map(tab => (
              <button key={tab.id} onClick={() => { setActiveTab(tab.id); setSidebarOpen(false) }} className={`w-full text-left px-3 py-2 rounded-lg text-sm transition-colors ${activeTab === tab.id ? "bg-white text-black font-bold" : "text-gray-400 hover:text-white hover:bg-gray-800"}`}>{tab.label}</button>
            ))}
          </nav>
        ...
        {/* Main Content */}
        <div className="flex-1 p-4 lg:p-6 overflow-auto max-h-screen pt-16 lg:pt-6">
          ...
          {activeTab === "dashboard" && (
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                ].map((s, i) => (<div key={i} className={`${s.bg} border ${s.b} rounded-2xl p-4 lg:p-6`}><p className={`text-2xl lg:text-3xl font-black ${s.c}`}>{s.v}</p><p className="text-xs text-gray-500 mt-1">{s.l}</p></div>))}
              </div>
          ...
          {activeTab === "matches" && (
            <div className="space-y-4">
              ...
              {(showAddForm || editingMatch) && (
                <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="bg-gray-900/80 border border-gray-700/60 rounded-2xl p-4 lg:p-6">
          ...
          {activeTab === "blog" && (
            <div className="space-y-4">
              ...
              {(showAddBlog||editingBlog)&&(<div className="bg-gray-900/80 border border-gray-700/60 rounded-2xl p-4 lg:p-6"><h3 className="text-sm font-bold text-white mb-4">{editingBlog?"Edit Post":"Add New Post"}</h3><form onSubmit={editingBlog?handleUpdateBlog:handleAddBlog} className="grid grid-cols-2 gap-3">
